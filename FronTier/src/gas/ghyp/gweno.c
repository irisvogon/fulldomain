...
LOCAL	void	g_update_point_ghost_cell(int,int, int*, bool,int,float**, float*, float*,float*,int,int,int,Stencil*,Front*);
	...
	int		dim)
{
	...
	start = sten_rad + 1;
        ...
        if(debugging("hypxiaoxue"))
        {
            for(j = 0; j < vsize; j++)
            {
                printf("dens[%d] = %f\n", j, vst->rho[j+offset]);
                printf("Energy[%d] = %f\n", j, vst->en_den[j + offset]);
            }
            for(j = 0 ; j  < vsize + 2; j++)
            {
                for(i = 0; i < kmax; i++)
                  printf("u at point [%d] physical [%d] : u = %f u1 = %f u2 = %f \n", j,i, u[i][j], uu1[i][j], uu2[i][j]);
            }
        }
        ...
        else //oned_WENO is called by point_FD
	{
	    ...
	    int stencil_length = vsize + 3*ubuff - 2;
            ...
            bool upboundary; // Whether we are calling point_FD at the upboundary*/
	    upboundary = YES;
            COMPONENT	*sten_comp = sten->compstore;
	    bool inflow = NO;
	    bool nozzle = NO;
	    int nozzle_indx;//point in the stencil above 3.90
	    int inflow_indx;//point in the stencil that is on the left of the computational domain
		...
		inflow = YES;
		  ...
		  inflow_indx = 2;
            ...
            float small = 0.001;
	    if(Coords(sten->pstore[3])[2] > 3.9 + small && Coords(sten->pstore[3])[0]*Coords(sten->pstore[3])[0] + Coords(sten->pstore[3])[1]*Coords(sten->pstore[3])[1] < 0.099*0.099 )
	    {
		nozzle = YES;
		if(Coords(sten->pstore[2])[2] > 3.9 + small) 
                        nozzle_indx = 2;
		else 
                        nozzle_indx = 3;
	    }
	    ...
	    {
                if(idirs[0] == 2 && Reflect_wall(vst->state[1]) 
                || idirs[0] == 1 && Reflect_wall_y(vst->state[1]) 
                || idirs[0] == 2 && Ramp_cellz(vst->state[1]) )//point_FD update the second cell off the wall
                { 
		    num_cells_in_wall = 1;
		    if(idirs[0] == 2 && Reflect_wall(vst->state[1]) || idirs[0] == 1 && Reflect_wall_y(vst->state[1]))
                         boundary_cell = 1;
		}
		else if( idirs[0] == 2 && Reflect_wall(vst->state[2]) 
                      || idirs[0] == 1 && Reflect_wall_y(vst->state[2]) 
                      || idirs[0] == 2 && Ramp_cellz(vst->state[2]))//point_FD update the first cell off the wall
		{
		    num_cells_in_wall = 2;		    
		    if(idirs[0] == 2 && Reflect_wall(vst->state[2]) || idirs[0] == 1 && Reflect_wall_y(vst->state[2]))
                         boundary_cell = 1;
		}
		else
	    ...
	    {
                if (idirs[0] == 2 && Reflect_wall(vst->state[stencil_length - 1 - 1]) 
                 || idirs[0] == 1 && Reflect_wall_y(vst->state[stencil_length - 1 - 1]) 
                 || idirs[0] == 0 && Ramp_cellx(vst->state[stencil_length - 1 - 1]))//point_FD update the second cell off the wall
		{
		    num_cells_in_wall = 1;
		    if(idirs[0] == 2 && Reflect_wall(vst->state[stencil_length - 1 - 1]) || idirs[0] == 1 && Reflect_wall_y(vst->state[stencil_length - 1 - 1]))
                         boundary_cell = 1;
		}
		else if(idirs[0] == 2 && Reflect_wall(vst->state[stencil_length - 1 - 2]) 
                     || idirs[0] == 1 && Reflect_wall_y(vst->state[stencil_length - 1 - 2]) 
                     || idirs[0] == 0 && Ramp_cellx(vst->state[stencil_length - 1 - 2]))//point_FD update the first cell off the wall
		{
		    num_cells_in_wall = 2;
		    if(idirs[0] == 2 && Reflect_wall(vst->state[stencil_length - 1 - 2]) || idirs[0] == 1 && Reflect_wall_y(vst->state[stencil_length - 1 - 2]))
                        boundary_cell = 1;
		}
		else
	    ...
	    float **u, **uu1, **uu2, **F_mid, **F_mid_nozzle;
	    float *p, *GAM, *c;
	    ...
	    bi_array( &F_mid ,u_length + 1, kmax, sizeof(float));
            bi_array(&F_mid_nozzle ,u_length + 1, kmax, sizeof(float));            
		...
		indx_ans_vst = 3*ubuff;
            ...
            float rate = 0;
            int cover = -1;
            if( upboundary && Coords(sten->pstore[3])[2] > 3.9) 
                compute_square_circle(0, 0, 0.099, Coords(sten->pstore[3])[0], Coords(sten->pstore[3])[1], fr->rect_grid->h[0], fr->rect_grid->h[1], &cover, &rate);
            
            if(upboundary)
                Fmid_indx = u_length - 3;
            else
                Fmid_indx = 3; 
            /* Step 1 of RK3 */
		g_update_point_ghost_cell(btype,swp_num,iperm,upboundary, kmax,u, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
		update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[0], endp[0], kmax, offset, u, p, c, GAM);
		    ...
		    for ( j = startx[0]; j < endx[0]; j++)
                ...
                else
                {
                    if(nozzle)
                    {
                        g_update_point_ghost_cell(2,swp_num,iperm,upboundary, kmax,u, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
                        update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[0], endp[0], kmax, offset, u, p, c, GAM);
                        compute_weno_flux(fr,vst,startx[0],endx[0],sten_rad,vsize,num_comps,offset,idirs, u, p, GAM, c, F_mid_nozzle);
                        
                        if(boundary_cell){for(i = 0; i < kmax; i++){if( i!= 1) F_mid_nozzle[Fmid_indx][i] = 0;}}

                        for(i = 0; i < kmax; i++)
                            F_mid[Fmid_indx][i] = (1- rate)*F_mid[Fmid_indx][i]   +  rate*F_mid_nozzle[Fmid_indx][i]; 
 		        ...
 		        for (j = startx[0]; j < endx[0]; j++)
                    ...
                    {
                        g_update_point_ghost_cell(1,swp_num,iperm,upboundary, kmax,u, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
                        update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[0], endp[0], kmax, offset, u, p, c, GAM);
                        compute_weno_flux(fr,vst,startx[0],endx[0],sten_rad,vsize,num_comps,offset,idirs, u, p, GAM, c, F_mid_nozzle);
                        ...
                        for(i = 0; i < kmax; i++)
                            F_mid[Fmid_indx][i] = rate*F_mid[Fmid_indx][i]   + (1 - rate)*F_mid_nozzle[Fmid_indx][i] ;
 		        ...
 		        for ( j = startx[0]; j < endx[0]; j++)
                    ...
                    if(debugging("nozzlex"))
                    {
                          printf("\n");
                          printf("nozzle = %d update point[%f %f %f] rate = %f cover = %d Fmid_indx= %d \n", nozzle,Coords(sten->pstore[3])[0], 
                                  Coords(sten->pstore[3])[1], Coords(sten->pstore[3])[2], rate, cover, Fmid_indx);
 		          for ( j = startx[0]; j < endx[0]+1; j++)
                              printf("1. F_mid[%d] = [%f %f %f %f %f] F_mid_nozzle = [%f %f %f %f %f]\n", j,
                                      F_mid[j][0], F_mid[j][1],F_mid[j][2],F_mid[j][3],F_mid[j][4],
                                      F_mid_nozzle[j][0],F_mid_nozzle[j][1], F_mid_nozzle[j][2],F_mid_nozzle[j][3],F_mid_nozzle[j][4]);
                          printf("\n");

                          for( j = 0; j < u_length; j++)
                              printf(" u[%d] = %f [%f %f %f] %f\n", j, u[0][j], u[1][j]/u[0][j], u[2][j]/u[0][j], u[3][j]/u[0][j], u[4][j]);
                          printf("\n");
                          for( j = 0; j < u_length; j++)
                              printf(" uu1[%d] = %f [%f %f %f] %f\n", j, uu1[0][j], uu1[1][j]/uu1[0][j], uu1[2][j]/uu1[0][j], uu1[3][j]/uu1[0][j], uu1[4][j]);
                          printf("\n");
                    }
                }
    
            /* Step 2 of RK3 */
	   	g_update_point_ghost_cell(btype,swp_num,iperm,upboundary,kmax,uu1,p,GAM,c,num_ghost_cell, fcell,lcell,sten,fr);
		...
		compute_weno_flux(fr, vst,startx[1],endx[1], sten_rad, vsize, num_comps, offset, idirs, uu1, p, GAM, c, F_mid);
		    ...
		    for ( j = startx[1]; j < endx[1]; j++)
                ...
                else
                {
                    if(nozzle)
                    {
                        g_update_point_ghost_cell(2,swp_num,iperm,upboundary, kmax,uu1, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
                        update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[1], endp[1], kmax, offset, uu1, p, c, GAM);
                        compute_weno_flux(fr,vst,startx[1],endx[1],sten_rad,vsize,num_comps,offset,idirs, uu1, p, GAM, c, F_mid_nozzle);
 		        ...
 		        for ( j = startx[1]; j < endx[1]; j++)
                    ...
                    {
                        g_update_point_ghost_cell(1,swp_num,iperm,upboundary, kmax,uu1, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
                        update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[1], endp[1], kmax, offset, uu1, p, c, GAM);
                        compute_weno_flux(fr,vst,startx[1],endx[1],sten_rad,vsize,num_comps,offset,idirs, uu1, p, GAM, c, F_mid_nozzle);
 		          ...
 		          for ( j = startx[1]; j < endx[1]+1; j++)
                              printf("2. F_mid[%d] = [%f %f %f %f %f] F_mid_nozzle = [%f %f %f %f %f]\n", j,
                                      F_mid[j][0], F_mid[j][1],F_mid[j][2],F_mid[j][3],F_mid[j][4],
                                      F_mid_nozzle[j][0],F_mid_nozzle[j][1], F_mid_nozzle[j][2],F_mid_nozzle[j][3],F_mid_nozzle[j][4]);
                          printf("\n");
                          
                          for( j = 0; j < u_length; j++)
                              printf(" uu2[%d] = %f [%f %f %f] %f\n",j, uu2[0][j], uu2[1][j]/uu2[0][j], uu2[2][j]/uu2[0][j], uu2[3][j]/uu2[0][j], uu2[4][j]);
                          printf("\n");
            ...
            /* Step 3 of RK3 */
	   	g_update_point_ghost_cell(btype,swp_num, iperm, upboundary, kmax,uu2, p, GAM, c, num_ghost_cell, fcell,lcell, sten,fr);
		    ...
		    for ( j = startx[2]; j < endx[2]; j++)
                ...
                else
                {
                    if(debugging("nozzlex"))
                    {
                          ...
                          printf("\n");
                          j = indx_ans_u;
                          printf(" return u[%d] = [%f %f %f %f %f]\n", j, u[0][j], u[1][j]/u[0][j], u[2][j]/u[0][j], u[3][j]/u[0][j], u[4][j]);
                    }
                    if(nozzle)
                    {
                        g_update_point_ghost_cell(2,swp_num,iperm,upboundary, kmax,uu2, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
                        update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[2], endp[2], kmax, offset, uu2, p, c, GAM);
                        compute_weno_flux(fr,vst,startx[2],endx[2],sten_rad,vsize,num_comps,offset,idirs, uu2, p, GAM, c, F_mid_nozzle);
 		        ...
 		        for ( j = startx[2]; j < endx[2]; j++)
                    ...
                    {
                        g_update_point_ghost_cell(1,swp_num,iperm,upboundary, kmax,uu2, p, GAM, c, num_ghost_cell,fcell,lcell,sten,fr);
                        update_pressure_soundspeed_gamma(fr, vst, indx_ans_vst, startp[2], endp[2], kmax, offset, uu2, p, c, GAM);
                        compute_weno_flux(fr,vst,startx[2],endx[2],sten_rad,vsize,num_comps,offset,idirs, uu2, p, GAM, c, F_mid_nozzle);
 		        ...
 		        for ( j = startx[2]; j < endx[2]; j++)
 		          ...
 		          for ( j = startx[2]; j < endx[2]+1; j++)
                              printf("3. F_mid[%d] = [%f %f %f %f %f] F_mid_nozzle = [%f %f %f %f %f]\n", j,
                                      F_mid[j][0], F_mid[j][1],F_mid[j][2],F_mid[j][3],F_mid[j][4],
                                      F_mid_nozzle[j][0],F_mid_nozzle[j][1], F_mid_nozzle[j][2],F_mid_nozzle[j][3],F_mid_nozzle[j][4]);
                          printf("\n");
		...
		Front	*fr)
{
        ...
        if(btype == 0)
	{
		...
		for( j = 0 ; j < kmax; j++)
		    u[j][i] = u[j][num_ghost_cell]; //copy the first inflow state to all the outer states
	    return;
	}
        else if(btype == 1)
	{
		...
		for(i = 0; i < 10; i++)
			pdens(sl)[i] = 0.0;
		pdens(sl)[4] = Dens(sl);
		Set_params(sl,sten->ststore[2]);	
		int st_type = state_type(sten->ststore[2]);
        ...
        else if(idirs[0] == 0 && upboundary ) // assign the ghost cell inside the ramp for the x direction sweep
	{
	    nvec[0] = -sin10;
	    nvec[1] = 0.0;
	    nvec[2] = cos10;
	...
	else if(idirs[0] == 2 && !upboundary && (Coords(sten->pstore[2])[0] < ramp_x0 || Coords(sten->pstore[2])[0] > ramp_x1) || !upboundary && idirs[0] == 1)
	{
	...
	else if(idirs[0] == 2 && !upboundary && Coords(sten->pstore[2])[0] > ramp_x0 && Coords(sten->pstore[2])[0] < ramp_x1)
	{
	    nvec[0] = -sin10;
	    nvec[1] = 0.0;
	    nvec[2] = cos10;
	...
	else if((idirs[0] == 2 || idirs[0] == 1) && upboundary) // sweep in z direction
	{
	...
	Front		*front)
{
	...
	/*Boundary layer treatment for up wall, low wall and ramp*/
        for (i0 = imin[0]; i0 < imax[0]; i0++)
        {
			...
			compute_square_circle(0,0,0.099,coords[0],coords[1],2*dh[0],2*dh[1],&cover_by_nozzle, &rate);
			for( i = 0; i < 3; i++)
		  ...
		  if(debugging("negdens"))
                 {
                      if( ( fabs(coords[0] - 7.78) < 0.05 || fabs(coords[0] - 7.88) < 0.05) && fabs(coords[1] - 0.0505) < 0.05 && fabs(coords[2] - 2.48) < 0.05)
                      {
                        int npoint = 0;
                        if(fabs(coords[0] - 7.78) < 0.05)
                            npoint = 0;
                        else
                            npoint = 1;
                        printf(" in update_merge_cell npoint = %d coords = [%f %f %f]\n", npoint, coords[0], coords[1], coords[2]);
                        printf(" rho_old = %f rho = %f drho = %f\n", Dens(state), Dens(state), 0);
                        printf(" u_old = %f u = %f du = %f\n",  Mom(state)[0]/Dens(state), (Mom(state)[0] + flux[1]*dt/dh[2]) /Dens(state), flux[1]*dt/dh[2]*1.0/Dens(state)); 
                        printf(" mom_old = %f mom = %f dmom = %f\n", Mom(state)[0], Mom(state)[0] + flux[1]*dt/dh[2], flux[1]*dt/dh[2]);
                        printf("\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ ");
                        printf("\n");
                      }
                 }
			...
			flux[i] = 0.0;
 		        yw = yd;
			y0 = yd + delta;
            //            printf(" coords = [%f %f %f ] yw = %f dens =  %f Energy = %f Mom = [%f %f %f]\n is_obstacel_state(newstate) = %d\n", coords[0], coords[1], coords[2], yw, Dens(newstate), Energy(newstate), Mom(newstate)[0], Mom(newstate)[1],Mom(newstate)[2], is_obstacle_state(newstate)); 
...
//	      if(coords[0]> (front->rect_grid->GU[0]-dh) && !is_obstacle_state(st))
              //xiaoxue
              if(debugging("toppressure"))
              {
                  if(coords[0]> (front->rect_grid->GU[0]-dh) && coords[2] > 2.4 + dh &&  coords[2] < 3.9 - dh  && !is_obstacle_state(st))
	          {
		    st_type = state_type(st);
...
LOCAL	void compute_square_circle(
		...
		double *rate)//ratio of the cell out of the nozzle
{
	...
	for( i = 0; i < 2; i++)
	{
		...
		if(sdir[i][j] == 1)
		{
			...
			*rate =  0.5*l0*l1/(xl*yl) + arch_area/(xl*yl);
		   }
		   else if( cover[i][rj] == 1 && cover[ri][j] == 0)
		   {
			...
			arch_area = compute_arch_area(radius, archl);
			*rate = 1- ( 0.5*(l0 + l1)*xl + arch_area)/(xl*yl) ;
		   }
		   else if(cover[i][rj] == 0 && cover[ri][j] == 1)
		   {
			...
			arch_area = compute_arch_area(radius, archl);			
			*rate = 1 - (0.5*(l0+l1)*yl + arch_area)/(xl*yl);
		   }
		   else
		   {
			...
			l1 = sqrt(radius * radius - (sq[ri][rj][0] - xc)*(sq[ri][rj][0] - xc)) - fabs(sq[ri][rj][1] - yc);
                        archl = sqrt(l0*l0 + l1*l1);
			arch_area = compute_arch_area(radius, archl);
			*rate = 1 - (0.5*l0*l1 + arch_area)/(xl*yl);
		   }
		}
	    }
	}
